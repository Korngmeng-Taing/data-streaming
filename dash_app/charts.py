    ...
    for coin in sel_coins:
        ...
        if chart_type == "bar":
            fig.add_trace(go.Bar(
                x=cdf[tc], y=cdf[vc], name=coin.upper(), opacity=0.75,
            ...
            close_series = cdf[vc]
            high_series = np.maximum(cdf.get("max_price", cdf[vc]), np.maximum(open_series, close_series))
            low_series = np.minimum(cdf.get("min_price", cdf[vc]), np.minimum(open_series, close_series))
...
def build_technical_chart(cdf, coin, tc, vc, voc, cc):
    ...
    close_series = price
    high_series = np.maximum(cdf.get("max_price", price), np.maximum(open_series, close_series))
    low_series = np.minimum(cdf.get("min_price", price), np.minimum(open_series, close_series))
...
def build_volume_comparison_chart(vol_pivot):
    ...
    if not vol_pivot.empty and len(vol_pivot.columns) >= 1:
        fig = px.bar(vol_pivot.sum().reset_index(), x="coin_id", y=0, color="coin_id",
...
def build_returns_chart(returns_data):
    ...
    if returns_data:
        ...
        fig = px.box(ret_df, x="coin", y="return_pct", color="coin",
